...
from light import chunk_emit_light
from meshes.chunk_mesh_builder import get_chunk_index, get_index
from utils import get_light, numpyindex, get_xyz
...
class World:
    def __init__(self, app):
        ...
        self.torch_index = 0
        self.light_map = np.multiply(np.ones([WORLD_VOL, CHUNK_VOL]),  1)
        self.build_chunks()
    ...
    @staticmethod
    @njit
    def chunk_light(chunk_voxels):
        light_map = np.zeros(CHUNK_VOL)


        for index in numpyindex(chunk_voxels, LIGHT_BLOCKS):
            block = chunk_voxels[int(index)]
            x,y,z = get_xyz(index)
            light_map =  chunk_emit_light(chunk_voxels,light_map,(x, y, z), get_light(block))
        return  light_map
